...
return_array = zeros(10, MAX_DUR, MAX_SCEN)
...
# gapv gmdb
gapv_gmdb_life_factor = zeros(MAX_DUR) # GMDB max period = 54 * 12
...
lives_sr = zeros(MAX_DUR, MAX_SCEN) # GMDB max period = 54 * 12
av_sr = zeros(MAX_DUR, MAX_SCEN) # GMDB max period = 54 * 12
gv_sr = zeros(MAX_DUR, MAX_SCEN) # GMDB max period = 54 * 12
ga_sr = zeros(MAX_DUR, MAX_SCEN) # GMDB max period = 54 * 12
lives_ppa = zeros(MAX_DUR, MAX_SCEN) # GMDB max period = 54 * 12
av_ppa = zeros(MAX_DUR, MAX_SCEN) # GMDB max period = 54 * 12
gv_ppa = zeros(MAX_DUR, MAX_SCEN) # GMDB max period = 54 * 12
ga_ppa = zeros(MAX_DUR, MAX_SCEN) # GMDB max period = 54 * 12
for scen_num in 1:1000
    println("scenario ", scen_num)
    # decrement, account value and guarantee value
    lives_sr[1, scen_num] = 1.0
    av_sr[1, scen_num] = INITIAL_AV
    gv_sr[1, scen_num] = INITIAL_GV
    lives_ppa[1, scen_num] = 1.0
    av_ppa[1, scen_num] = INITIAL_AV
    gv_ppa[1, scen_num] = INITIAL_GV
    for t in (INITIAL_T + 1):(ACC_PERIOD * 12 - INITIAL_T + 2)
        ...
        av_sr[t, scen_num] = av_sr[t - 1, scen_num]
        if t == 2
            ...
            av_sr[t, scen_num] += SINGLE_PREMIUM * (1.0 - POLICY_CHARGE)
            ga_sr[t, scen_num] = -SINGLE_PREMIUM * COMM_RATE + SINGLE_PREMIUM * POLICY_CHARGE
        end
        av_sr[t, scen_num] = (1.0 + sum(return_weight .* return_array[:, cld(t - 1, 12), scen_num])) ^ (1.0 / 12) * (av_sr[t, scen_num] - gt - mg - cu - pw)
        gv_sr[t, scen_num] = gv_sr[t - 1, scen_num] * (1.0 - pw / av_sr[t, scen_num])
        ga_sr[t, scen_num] += gt + rb - exp - cu - qd * max(0, gv_sr[t, scen_num] - av_sr[t, scen_num])

        # sr lapse
        if t <= 13
            qw = 1.0 - (1.0 - min(lapse_rate_sr[1, 3], max(lapse_rate_sr[1, 2], lapse_rate_sr[1, 1] * (av_sr[t, scen_num] / gv_sr[t, scen_num]) ^ 3))) ^ (1.0 / 12)
        else
            qw = 1.0 - (1.0 - min(lapse_rate_sr[2, 3], max(lapse_rate_sr[2, 2], lapse_rate_sr[2, 1] * (av_sr[t, scen_num] / gv_sr[t, scen_num]) ^ 3))) ^ (1.0 / 12)
        end
        if av_sr[t, scen_num] <= 0.0
            lives_sr[t, scen_num] = 0.0
        else
            lives_sr[t, scen_num] = lives_sr[t - 1, scen_num] * (1.0 - qd) * (1.0 - qw)
        end
        ...
        av_ppa[t, scen_num] = av_ppa[t - 1, scen_num]
        if t == 2
            gt = SINGLE_PREMIUM * (1.0 - POLICY_CHARGE) * GUARANTEE_CHARGE[cld(ISSUE_AGE, 5) - 8, GENDER]
            mg = SINGLE_PREMIUM * (1.0 - POLICY_CHARGE) * (1.0 - GUARANTEE_CHARGE[cld(ISSUE_AGE, 5) - 8, GENDER]) * MANAGEMENT_CHARGE / 12
            cu = SINGLE_PREMIUM * (1.0 - POLICY_CHARGE) * (1.0 - GUARANTEE_CHARGE[cld(ISSUE_AGE, 5) - 8, GENDER]) * CUSTODIAN_CHARGE / 12
            ...
            exp += (STAB_FUND_RATE + PREM_TAX_RATE[1]) * gt + (REG_FEE_RATE + CONSUM_PROTECT_FEE_RATE) * (gt + rb) + PREM_TAX_RATE[2] * rb
            exp += (STAB_FUND_RATE + PREM_TAX_RATE[1]) * (SINGLE_PREMIUM * POLICY_CHARGE) + (REG_FEE_RATE + CONSUM_PROTECT_FEE_RATE) * (SINGLE_PREMIUM * POLICY_CHARGE)
            av_ppa[t, scen_num] += SINGLE_PREMIUM * (1.0 - POLICY_CHARGE)
            ga_ppa[t, scen_num] = -SINGLE_PREMIUM * COMM_RATE + SINGLE_PREMIUM * POLICY_CHARGE
        end
        av_ppa[t, scen_num] = (1.0 + sum(return_weight .* return_array[:, cld(t - 1, 12), scen_num])) ^ (1.0 / 12) * (av_ppa[t, scen_num] - gt - mg - cu - pw)
        gv_ppa[t, scen_num] = gv_ppa[t - 1, scen_num] * (1.0 - pw / av_ppa[t, scen_num])
        ga_ppa[t, scen_num] += gt + rb - exp - cu - qd * max(0, gv_ppa[t, scen_num] - av_ppa[t, scen_num])

        # ppa lapse
        # itm = lapse_mult[1] * max(SINGLE_PREMIUM, av_ppa[t, scen_num]) * gapv_gmdb_life_factor[t - 1]
        itm = lapse_mult[1] * gv_ppa[t, scen_num] * gapv_gmdb_life_factor[t - 1]
        if av_ppa[t, scen_num] > 0
            itm /= av_ppa[t, scen_num]
        end
        if itm < 0.5
            if t <= 37
        ...
        lives_ppa[t, scen_num] = lives_ppa[t - 1, scen_num] * (1.0 - qd) * (1.0 - qw)
    end
end
CSV.write("gmdb_lives_sr.csv", Tables.table(lives_sr), writeheader = false)
CSV.write("gmdb_av_sr.csv", Tables.table(av_sr), writeheader = false)
CSV.write("gmdb_gv_sr.csv", Tables.table(gv_sr), writeheader = false)
CSV.write("gmdb_ga_sr.csv", Tables.table(ga_sr), writeheader = false)
CSV.write("gmdb_lives_ppa.csv", Tables.table(lives_ppa), writeheader = false)
CSV.write("gmdb_av_ppa.csv", Tables.table(av_ppa), writeheader = false)
CSV.write("gmdb_gv_ppa.csv", Tables.table(gv_ppa), writeheader = false)
CSV.write("gmdb_ga_ppa.csv", Tables.table(ga_ppa), writeheader = false)
